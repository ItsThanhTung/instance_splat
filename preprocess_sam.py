...
def create(image_list, data_list, save_folder, visualized_folder):
    ...
    for i, img in tqdm(enumerate(image_list), desc="Embedding images", leave=False):
        timer += 1
        try:
            img_embed, seg_map, seg_image = _embed_clip_sam_tiles(img.unsqueeze(0), sam_encoder)
        except:
            raise ValueError(timer)

        image_folder = os.path.join(visualized_folder, ".".join(data_list[i].split(".")[:-1]))
        os.makedirs(image_folder, exist_ok=True)
        for key in seg_map.keys():
            segment_map = seg_map[key]
            unique_segments = np.unique(segment_map)
            unique_segments = unique_segments[unique_segments != -1]  # Exclude -1 from unique segments

            # Generate distinct colors using a colormap (e.g., HSV)
            cmap = plt.get_cmap('hsv', len(unique_segments))
            distinct_colors = cmap(np.linspace(0, 1, len(unique_segments)))[:, :3]
            np.random.shuffle(distinct_colors)

            # Create a dictionary to map each segment to a color
            color_map = {segment: color for segment, color in zip(unique_segments, distinct_colors)}

            # Set the background color to black
            color_map[-1] = [0, 0, 0]  # RGB for black
            colorized_image = np.zeros((segment_map.shape[0], segment_map.shape[1], 3))

            # Assign colors to each segment in the image, including the background
            for segment, color in color_map.items():
                colorized_image[segment_map == segment] = color
            colorized_image = (colorized_image * 255.0).astype(np.uint8)
            saved_image = Image.fromarray(colorized_image)
            saved_image.save(os.path.join(image_folder, f"image_{key}.png"))
...
def _embed_clip_sam_tiles(image, sam_encoder):
    ...
    return clip_embeds, seg_map, seg_images
...
if __name__ == '__main__':
    ...
    save_folder = os.path.join(dataset_path, "sam", 'language_features')
    os.makedirs(save_folder, exist_ok=True)
    visualized_folder = os.path.join(dataset_path, "sam", 'visualized_language_features')
    os.makedirs(save_folder, exist_ok=True)
    create(imgs, data_list, save_folder, visualized_folder)